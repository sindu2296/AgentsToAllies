"""
Concurrent GHC Travel Planner Demo - Microsoft Agent Framework

Demonstrates parallel processing of travel planning for Grace Hopper Conference 2025.
...
async def main():
    """
    Concurrent GHC Travel Planner Demo using Microsoft Agent Framework
    
    Processes travel planning concurrently: flights and hotels searched in parallel,
    then summary agent creates the final itinerary.
    """
    ...
    for i, user_query in enumerate(user_queries, start=1):
        print("\n" + "="*80)
        print(f"TRAVEL PLANNING REQUEST #{i} (CONCURRENT)")
        print("="*80)
        ...
        result = await run_concurrent_travel_workflow(chat_client, user_query)
